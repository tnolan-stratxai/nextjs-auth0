...
import { LEGACY_COOKIE_NAME } from "./normalize-session";
...
describe("Stateless Session Store", async () => {
    describe("get", async () => {
        ...
        describe("migrate legacy session", async () => {
            it("should convert the legacy session to the new format", async () => {
                ...
                const encryptedCookieValue = await encrypt(legacySession, secret, legacyHeader);
                const headers = new Headers();
    ...
    describe("delete", async () => {
        ...
        it("should not throw an error if the cookie does not exist", async () => {
            ...
            await expect(sessionStore.delete(requestCookies, responseCookies)).resolves.not.toThrow();
        });
    });
});
