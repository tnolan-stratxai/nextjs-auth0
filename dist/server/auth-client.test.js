...
-----END PRIVATE KEY-----`,
    ...
    function getMockAuthorizationServer({ tokenEndpointResponse, tokenEndpointErrorResponse, discoveryResponse, audience, nonce, keyPair = DEFAULT.keyPair, onParRequest } = {}) {
    ...
    describe("handleLogout", async () => {
        it("should redirect to the authorization server logout URL with the correct params", async () => {
            ...
            expect(authorizationUrl.searchParams.get("id_token_hint")).toEqual(DEFAULT.idToken);
    ...
    describe("handleAccessToken", async () => {
        it("should return the access token if the user has a session", async () => {
            ...
            const updatedSessionCookie = response.cookies.get("__session");
            const { payload: updatedSession } = await decrypt(updatedSessionCookie.value, secret);
    ...
    describe("startInteractiveLogin", async () => {
        const createAuthClient = async ({ pushedAuthorizationRequests = false, signInReturnToPath = "/", authorizationParameters = {} } = {}) => {
            const secret = await generateSecret(32);
            const transactionStore = new TransactionStore({
                secret
            });
    ...
    describe("getConnectionTokenSet", async () => {
        it("should call for an access token when no connection token set in the session", async () => {
            ...
            const response = await authClient.getConnectionTokenSet(tokenSet, undefined, { connection: "google-oauth2", login_hint: "000100123" });
        ...
        it("should return access token from the session when connection token set in the session is not expired", async () => {
            ...
            const response = await authClient.getConnectionTokenSet(tokenSet, {
                connection: "google-oauth2",
                accessToken: "fc_at",
        ...
        it("should call for an access token when connection token set in the session is expired", async () => {
            ...
            const response = await authClient.getConnectionTokenSet(tokenSet, { connection: "google-oauth2", accessToken: "fc_at", expiresAt }, { connection: "google-oauth2", login_hint: "000100123" });
            const [error, connectionTokenSet] = response;
            expect(error).toBe(null);
            expect(connectionTokenSet).toEqual({
                accessToken: DEFAULT.accessToken,
        ...
        it("should return an error if the discovery endpoint could not be fetched", async () => {
            ...
            const [error, connectionTokenSet] = await authClient.getConnectionTokenSet(tokenSet, undefined, {
                connection: "google-oauth2"
            });
            expect(error?.code).toEqual("discovery_error");
